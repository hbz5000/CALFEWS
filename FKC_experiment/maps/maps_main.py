...
figure_type = 'statewide'
use_type = 'paper'#for total & statewide figure types -
#total  - use_type: contract, land_use, crop_groups, wetlands, groundwater
#statewide  - use_type: paper, projects1, projects2, ...., projects9, projects10
...
for index, row in reservoir_shapefile.iterrows():
  ...
  data_figure.plot_scale(current_watershed, 'none', type = 'points', solid_color = reservoir_colors[counter], markersize_use = 40.0, solid_alpha = 1.0, zorder = 3)
  counter += 1
...
for shapefile_name, shapefile_color in zip(figure_attributes['shapefiles'], figure_attributes['shapefile_color']):
  ...
  if shapefile_name in base_list:
    ...
    outline_color = shapefile_color
    solid_alpha = 0.6
  data_figure.plot_scale(shapes, 'none', type = 'polygons', solid_color = shapefile_color, outline_color = outline_color, linewidth_size = lw, zorder = zorder_int, solid_alpha = solid_alpha)
...
if figure_type == 'total':
  ...
  if use_type == 'paper':
    ...
    for label_name, offset_x, offset_y in zip(label_list_big, offset_x_list, offset_y_list):
      ...
      data_figure.label_plot(reservoir_labels, longitude, latitude, label, name, label_list, offset_x, offset_y)
    ...
    for label_name, offset_x, offset_y, label_writing in zip(label_list_big, offset_x_list, offset_y_list, label_name_big):
      label_list = []
      label_list.append(label_name)
      data_figure.label_plot(reservoir_labels, longitude, latitude, label, name, label_list, offset_x, offset_y, map_text_alt = label_writing)
...
data_figure.add_inset_figure(shapefile_name, box_lim_x, box_lim_y, inset_lim_x, inset_lim_y)

#plt.savefig(project_folder + '/finished_maps/' + figure_type + '_' + use_type + '_1800.png', dpi = 1800, bbox_inches = 'tight', pad_inches = 0.0)
plt.savefig(project_folder + '/finished_maps/' + figure_type + '_' + use_type + '_300.png', dpi = 300, bbox_inches = 'tight', pad_inches = 0.0)
plt.show()
