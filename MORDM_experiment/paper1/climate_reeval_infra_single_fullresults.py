...
if __name__ == "__main__":
    ...
    base_results_folder = 'results/climate_reeval_single_fullresults/'

    soln_label = 's3/soln212'
    
    ### define climate projections
    scenarios = glob('calfews_src/data/CA_FNF_climate_change/CA_FNF_*.csv')
    ### drop canesm2 scenarios, which have a data inconsistency issue. just use one scenario here.
    scenarios = [s for s in scenarios if 'canesm2' not in s][:1]
    ...
    for i, flow_input_source in enumerate(flow_input_sources):
        print(f'Starting {soln_label}, {flow_input_source}')
        ...
        a = main_cy_obj.run_sim_py(overall_start_time)
