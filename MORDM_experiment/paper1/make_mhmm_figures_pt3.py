...
for figmode in ['past','future']:
    ...
    alpha = 1
    # for ax, mhmm_datasets in zip([axs[1]], ['./mhmm_data/MIL_TL.csv']):
    for ax, mhmm_datasets in zip(axs, ['./mhmm_data/AboveMIL_TL.csv', './mhmm_data/MIL_TL.csv', './mhmm_data/BelowMIL_TL.csv']):
        ...
        for i in range(32):
            ...
            ax.plot(exceedence, sort, color="navy", alpha=alpha, lw=1,
                    label="Synthetic scenarios" if i == 0 else "", zorder=1)#zorder[i])
        ### scenarios 33-96 used in reevaluation step
        for i in range(33, 97):
            sort = np.sort(sample.iloc[:, i])
            exceedence = np.arange(1., len(sort) + 1) / (len(sort) + 1)
            ax.plot(exceedence, sort, color="navy", alpha=alpha, lw=1, label="", zorder=1)#zorder[i])
    ...
    for i,ds in enumerate(datasets):
        ...
        for r in missing_rows:
            df.iloc[r,:] = (df.iloc[r-1,:].values + df.iloc[r+1,:].values) / 2
