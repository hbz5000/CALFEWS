...
class Visualizer():
  ...
  def __init__(self, district_list, private_list, city_list, contract_list, bank_list, leiu_list):
    ...
    inflow_inputs = pd.read_csv('cord/data/input/cord-data.csv', index_col=0, parse_dates=True)
    x2_results = pd.read_csv('cord/data/input/x2DAYFLOW.csv', index_col=0, parse_dates=True)
    ...
    kern_bank_observations = pd.read_csv('cord/data/input/kern_water_bank_historical.csv')
    kern_bank_observations = kern_bank_observations.set_index('Year')
    semitropic_bank_observations = pd.read_csv('cord/data/input/semitropic_bank_historical.csv')
        ...
        for risk_alpha in range(0, 10):
          ...
          for dowy in range(0, n_colors):
            ...
            if np.sum(np.power(predictor, 2)) > 0.0:
              ...
              x_cord = [min(predictor) - predictor_std * risk_alpha / range_sensitivity, max(predictor) + predictor_std * risk_alpha / range_sensitivity]
              y_cord = [(min(predictor) - predictor_std * risk_alpha / range_sensitivity) * coef[0] + coef[1], (max(predictor) + predictor_std * risk_alpha / range_sensitivity) * coef[0] + coef[1]]
              ax0.plot(x_cord, y_cord, color = colors[dowy], zorder = 3, linewidth = 3, alpha = (10.0 - risk_alpha)/ 10.0)
              if risk_alpha == 0:
                if counter == scatter_interval:
