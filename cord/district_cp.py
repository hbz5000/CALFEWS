...
class District():
  ...
  def __init__(self, model, name, key, scenario_file = 'baseline'):
    self.T = model.T
    # self.starting_year = model.starting_year
    # self.number_years = model.number_years
    ...
    self.contract_list_length = len(self.contract_list)
...
#####################################################################################################################
  ...
  def find_baseline_demands(self, wateryear, days_in_month, non_leap_year):
    self.monthlydemand = {}
    wyt_list = ['W', 'AN', 'BN', 'D', 'C']
  ...
  def calc_demand(self, wateryear, year, da, m, m1, wyt, days_in_month):
    ...
    for irrseason in range(6,9):
      self.irrseasondemand += max(self.monthlydemand[wyt][irrseason]*days_in_month[year][irrseason], 0.0)
    if m > 9:
      for monthloop in range(m, 12):
        self.annualdemand += max(self.monthlydemand[wyt][monthloop]*days_in_month[year][monthloop],0.0)
      for monthloop in range(0,9):
        self.annualdemand += max(self.monthlydemand[wyt][monthloop]*days_in_month[year+1][monthloop], 0.0)
    else:
...
#####################################################################################################################
  ...
  def open_recovery(self,t, dowy, wateryear, target_eoy):
    ...
    total_balance = 0.0
    self.recovery_capacity_remain = (366-dowy)*self.max_recovery + self.extra_leiu_recovery
    ...
    if (total_balance + self.recovery_capacity_remain) < total_needs + target_eoy:
      if existing_carryover > 0.0:
        self.use_recovery = 0.0
      else:
        if total_needs > 0.0:
          self.use_recovery = min(max(self.recovery_capacity_remain/total_needs, 0.0), 1.0)
        else:
          self.use_recovery = 0.0
    else:
      self.use_recovery = 0.0
  ...
  def open_recharge(self,t,m,da,wateryear,year,numdays_fillup, numdays_fillup2, contract_carryover, key, wyt, reachable_turnouts, additional_carryover, contract_allocation, days_in_month):
  ...
  def set_turnback_pool(self, key, year, days_in_month):
...
#####################################################################################################################
  ...
  def find_leiu_output(self, contract_list, ownership, member_name, wateryear):
    ...
    total_contract = np.zeros(self.contract_list_length)
    if self.in_leiu_banking:
      bank_counter = 0
      ...
      if member_constraint > 0.0:
        for bank_contract_counter in range(self.contract_list_length):
