...
class Model():
  ...
  def __init__(self, input_data_file, expected_release_datafile, model_mode, demand_type, sensitivity_sample_number=-1, sensitivity_sample_names=[], sensitivity_sample=[], sensitivity_factors = None):
    ...
    self.first_d_of_month = first_d_of_month(self.dowy_eom, days_in_month(year_list, self.leap))
    ...
    self.short_day_year = self.short_index.dayofyear
    self.short_day_month = self.short_index.day
    self.short_month = self.short_index.month
    self.short_year = self.short_index.year
    ...
    self.short_days_in_month = days_in_month(short_year_list, leap(short_year_list))
...
#####################################################################################################################
  ...
  def initialize_water_districts(self, scenario = 'baseline'):
    ...
    for x in self.district_list:
      x.find_baseline_demands(0, self.days_in_month, self.non_leap_year)
    for x in self.private_list:
      x.find_baseline_demands(self.days_in_month, self.non_leap_year)
      x.turnout_list = {}
...
#####################################################################################################################
    ...
    for x in self.private_list:
      ...
      for land_keys in x.district_list:
        ...
        if district_land.has_pesticide:
          x.has_pesticide[land_keys] = True
          x.has_pmp[land_keys] = False
        else:
          x.has_pesticide[land_keys] = False
          if district_land.has_pmp:
            x.has_pmp[land_keys] = True
          else:
            x.has_pmp[land_keys] = False
      for land_keys in x.district_list:
        x.acreage[land_keys] = {}
  ...
  def determine_recharge_recovery_risk(self):
    ...
    total_available_length = len(total_available)
    for x in self.private_list:
      ...
      for district in x.district_list:
          ...
          for contracts in district_object.contract_list:
            ...
            for future_year in range(self.number_years):
      ...
      x.delivery_risk = np.zeros(total_available_length)
      x.delivery_risk_rate = np.zeros(total_available_length)
      cumulative_balance = 0.0
      cumulative_years = 0.0
      total_delivery_record = np.zeros(total_available_length)
      for hist_year in range(total_available_length):
    ...
    for w in self.waterbank_list:
      for member in w.participant_list:
        for irr_district in self.district_keys[member]:
          irr_district.max_recovery += w.ownership[member]*w.recovery
    #same for 'in leiu' banks (districts)
    for w in self.leiu_list:
      for member in w.participant_list:
        if member != w.key:
          for irr_district in self.district_keys[member]:
            irr_district.max_recovery += w.leiu_ownership[member]*w.leiu_recovery
  ...
  #     sum_ownership = 0
  #     for member in w.participant_list:
  #       self.district_keys[member].max_recovery += w.ownership[member]*w.recovery
  #   #same for 'in leiu' banks (districts)
  #   for w in self.leiu_list:
  #     for member in w.participant_list:
  #       if member != w.key:
  ...
  def project_urban(self, datafile, datafile_cvp, datafile_pumping):
    ...
    for xx in range(0, len(self.southbay.hist_pumping)):
      ...
      year = index_urban_y[xx]-urban_startYear
      self.southbay.hist_pumping[xx] += df_urban_monthly_cvp['PCH_pump'][m-1 + wateryear*12]*1000.0/urban_days_in_month[year][m-1]
...
#####################################################################################################################
  ...
  def simulate_north(self,t,swp_release, cvp_release, swp_release2, cvp_release2, swp_pump, cvp_pump):
    ...
    for x in [self.newmelones, self.donpedro, self.exchequer]:
      x.release_environmental(t,d,m,dowy,year, self.first_d_of_month,self.delta.forecastSJWYT)
	#Sacramento Tributaries
    self.oroville.set_oct_nov_rule(t, m)
    for x in [self.shasta, self.oroville, self.yuba, self.folsom]:
      x.release_environmental(t,d,m,dowy,year, self.first_d_of_month, self.delta.forecastSCWYT)
    ...
    self.exchequer.din, self.donpedro.din, self.newmelones.din  = self.delta.calc_vernalis_rule(t, d, self.year[t],m,dowy, NMI)
    ...
    for x in [self.shasta, self.folsom, self.oroville, self.yuba]:
      x.find_flow_pumping(t, m, dowy, year, self.delta.forecastSCWYT, 'env', self.days_in_month, self.dowy_eom)
    ...
    #OMR rule limits
    cvp_max, swp_max = self.delta.meet_OMR_requirement(t, m, dowy, year, cvp_max, swp_max)
    ...
    cvp_max_final, swp_max_final = self.delta.find_release(t, m, dowy, year, self.days_in_month, self.dowy_eom, cvp_max, swp_max, cvp_available_storage, swp_available_storage, cvp_release, swp_release, proj_surplus, max_pumping)
    ...
    #swp_forgone, swp_max_final = self.delta.hypothetical_pumping(swp_max, swp_max_final, swp_release2, 0.45, t, m)
    ...
    self.delta.step(t, d, m, dowy, da, wateryear, year, cvp_stored_flow, swp_stored_flow, swp_pump, cvp_pump, swp_available_storage, cvp_available_storage)
  ...
  def simulate_south(self, t, hro_pump, trp_pump, swp_alloc, cvp_alloc, proj_surplus, max_pumping, swp_forgone, cvp_forgone, swp_AF, cvp_AF, swp_AS, cvp_AS, wyt, wytSC, max_tax_free, flood_release, flood_volume):
    ...
    for x in watershed_reservoir_list:
      x.rights_call(x.downstream[t])
      x.release_environmental(t,d,m,dowy,year, self.first_d_of_month,wyt)
    ...
    for x in self.district_list:
      x.calc_demand(wateryear, year, da, m, m1, wyt, self.days_in_month)
      if x.has_private:
        x.private_demand = {}
        x.private_delivery = {}
    for x in self.private_list:
      x.calc_demand(wateryear, year, da, m, m1, wyt, self.days_in_month, self.non_leap_year)
    ...
    for x in self.urban_list:
      x.get_urban_demand(t, m, da, wateryear, year, self.forecastSRI[t], dowy, self.swp_allocation[t], allocation_change, self.model_mode, self.dowy_eom)
    for x in self.city_list:
      x.get_urban_demand(t, m, da, wateryear, year, self.forecastSRI[t], dowy, self.swp_allocation[t], allocation_change, self.model_mode, self.dowy_eom)
    #else:
    ...
    if m == 10 and da == 1:      
      ...
      for x in self.district_list:
        x.find_baseline_demands(wateryear, self.days_in_month, self.non_leap_year)
      for x in self.private_list:
        x.find_baseline_demands(self.days_in_month, self.non_leap_year)
	  
      for x in self.urban_list:
        x.find_pre_flood_demand(wyt, year, self.days_in_month)
    ...
    expected_pumping = self.estimate_project_pumping(t, dowy, year, proj_surplus, max_pumping, swp_AS, cvp_AS, self.max_tax_free, flood_release, wytSC)
    ...
    if m == 6 and da == 1:
      for y in self.contract_list:
        ...
        buyer_total = 0.0
        for x in self.district_list:
          seller_turnback, buyer_turnback = x.set_turnback_pool(y.name, year, self.days_in_month)
        ...
        for x in self.city_list:
          ...
          seller_turnback, buyer_turnback = x.set_turnback_pool(y.name, year, additional_carryover, self.days_in_month)
          for xx in x.district_list:
    ...
    for reservoir in [self.success, self.kaweah, self.isabella, self.pineflat, self.millerton]:
      reservoir.find_flow_pumping(t, m, dowy, year, wyt, 'demand', self.days_in_month, self.dowy_eom)
...
#find the storage pool for each contract
    ...
    use_tolerance = False
    for x in self.private_list:
      x.open_recovery(t, dowy, wyt, wateryear, use_tolerance, 0.0, self.number_years)
    use_tolerance = False
    for x in self.city_list:
      for xx in x.district_list:
        district_object = self.district_keys[xx]
      ...
      if dowy > 273:
        ...
        additional_carryover = 0.0
      x.open_recovery(t, dowy, wyt, wateryear, use_tolerance, additional_carryover, self.number_years)
    flow_type = "recovery"
    ...
    for x in self.district_list:
      for y in x.contract_list:
        ...
        x.open_recharge(t, m-1, da, wateryear, year, carryover_days, lookahead_days, contract_object.tot_carryover - contract_object.annual_deliveries[wateryear], y, wyt, self.contract_turnouts[y], 0.0,contract_object.allocation_priority, self.days_in_month)
    ...
    for x in self.private_list:
      for y in x.contract_list:
        ...
        x.open_recharge(t, m-1, da, wateryear, year, reservoir.numdays_fillup['demand'], lookahead_days, y, wyt, self.contract_turnouts[y], additional_carryover, contract_object.allocation_priority, self.days_in_month)
    for x in self.city_list:
      for y in x.contract_list:
        contract_object = self.contract_keys[y]
        reservoir = self.contract_reservoir[contract_object.key]
        ...
        x.open_recharge(t, m-1, da, wateryear, year, reservoir.numdays_fillup['demand'], lookahead_days, y, wyt, self.contract_turnouts[y], 0.0, contract_object.allocation_priority, self.days_in_month)
    ...
    for z in self.canal_list:
      counter = 0
      for canal_loc in range(len(self.canal_district[z.name])):
...
#####################################################################################################################		  
  ...
  def estimate_project_pumping(self, t, dowy, year, proj_surplus, max_pumping, swp_AS, cvp_AS, max_tax_free, flood_release, wyt):
  ...
  def find_pumping_release(self, start_storage, pump_max, month_demand, month_demand_must_fill, allocation, expected_pumping, flood_supply, available_storage, flood_storage, projected_carryover, current_carryover, max_tax_free, wyt, t, m, da, year, key):
  ...
  def project_urban_pumping(self, da, dowy, m, wateryear, year, total_delta_pumping, projected_allocation_cvp, sri):
        ...
        for wyt in ['W', 'AN', 'BN', 'D', 'C']:
          ...
          for y in self.city_list:
            for districts in y.district_list:
              ...
              #if monthcounter >= 9:
                #if wateryear > 0:
                  #y.monthlydemand[districts][wyt][monthcounter] += np.mean(district_object.hist_demand_dict['daily_fractions'][self.k_close_wateryear][start_of_month:start_next_month])*max(y.ytd_pumping[districts][wateryear-1], 0.0)
                #else:
                  #y.monthlydemand[districts][wyt][monthcounter] += np.mean(district_object.hist_demand_dict['daily_fractions'][self.k_close_wateryear][start_of_month:start_next_month])max(y.initial_pumping[districts], 0.0)
              #else:
                y.monthlydemand[districts][wyt][monthcounter] += np.mean(district_object.hist_demand_dict['daily_fractions'][self.k_close_wateryear][start_of_month:start_next_month])*max(sri_estimate - y.ytd_pumping[districts][wateryear], 0.0)
    ...
    for y in self.city_list:
      for districts in y.district_list:
        ...
        #sri_estimate = (total_delta_pumping*y.delivery_percent_coefficient[districts][dowy][0] + y.delivery_percent_coefficient[districts][dowy][1] - y.regression_errors[districts][dowy][self.k_close_wateryear])*total_delta_pumping
        sri_estimate = (total_delta_pumping*y.delivery_percent_coefficient[districts][dowy][0] + y.delivery_percent_coefficient[districts][dowy][1])*total_delta_pumping
        y.dailydemand[districts] += max(sri_estimate- y.ytd_pumping[districts][wateryear], 0.0)*district_object.hist_demand_dict['daily_fractions'][self.k_close_wateryear][dowy]
        y.dailydemand_start[districts] += max(sri_estimate -y.ytd_pumping[districts][wateryear], 0.0)*district_object.hist_demand_dict['daily_fractions'][self.k_close_wateryear][dowy]
    ...
    ##how long the banks have been used continuously
      ...
      for member in w.participant_list:
        for irr_district in self.district_keys[member]:
          ...
          irr_district.max_direct_recharge[0] += w.ownership[member]*w.recharge_rate
          for res in self.reservoir_list:
            if irr_district.reservoir_contract[res.key] == 1:
              res.max_direct_recharge[0] += w.ownership[member]*w.recharge_rate
          ...
          for m_counter in range(1,12):
            ...
            irr_district.max_direct_recharge[m_counter] += w.ownership[member]*w.recharge_rate*decline_coef
            for res in self.reservoir_list:
              if irr_district.reservoir_contract[res.key] == 1:
                res.max_direct_recharge[m_counter] += w.ownership[member]*w.recharge_rate*decline_coef
      ...
      if w.key == "SMI":
        for member in w.participant_list:
          ...
          #current direct recharge
            irr_district.max_direct_recharge[0] += w.leiu_ownership[member]*w.recharge_rate
          #current in leiu recharge (based on irrigation demands of the in-leiu bank)
            if w.inleiubanked[member] < w.inleiucap[member]:
              irr_district.max_leiu_recharge[0] += w.leiu_ownership[member]*w.monthlydemand[wyt][m-1]*(1.0 - min(bank_total_supply/w.annualdemand, 1.0))

            for res in self.reservoir_list:
              if irr_district.reservoir_contract[res.key] == 1:
                if w.inleiubanked[member] < w.inleiucap[member]:
                  res.max_direct_recharge[0] += w.leiu_ownership[member]*w.recharge_rate
          ...
          ##continuous use from this point on
            ...
            #future (12 months) direct recharge
              irr_district.max_direct_recharge[m_counter] += w.leiu_ownership[member]*w.recharge_rate*decline_coef
              #future (12 months) in leiu recharge
              if w.inleiubanked[member] < w.inleiucap[member]:
                irr_district.max_leiu_recharge[m_counter] += w.leiu_ownership[member]*w.monthlydemand[wyt][future_month]*(1.0 - min(bank_total_supply/w.annualdemand, 1.0))
              for res in self.reservoir_list:
                if irr_district.reservoir_contract[res.key] == 1:
                  if w.inleiubanked[member] < w.inleiucap[member]:
                    res.max_direct_recharge[m_counter] += w.leiu_ownership[member]*w.recharge_rate*decline_coef
  ...
  def distribute_canal_deliveries(self, dowy, canal, prev_canal, contract_canal, available_flow, canal_size, wateryear, flow_dir, flow_type, search_type):
    ...
    for x in self.private_list:
      for xx in x.district_list:
        ...
        district_object.private_demand[x.key] = private_demand_constraint
        district_object.private_delivery[x.key] = x.set_request_to_district(private_demand_constraint,search_type,contract_list,0.0,dowy,xx)
...
#####################################################################################################################
  ...
  def update_regulations_north(self,t,dowy,y):
    ...
    if y == 2014 and dowy == 1:
      ...
      #expected_outflow_req, expected_depletion = self.delta.calc_expected_delta_outflow(self, self.shasta.downstream,self.oroville.downstream,self.yuba.downstream,self.folsom.downstream, self.shasta.temp_releases, self.oroville.temp_releases, self.yuba.temp_releases, self.folsom.temp_releases)
      #expected_outflow_req = self.delta.min_outflow
      #expected_outflow_req['EC'] = expected_outflow_req['C']
