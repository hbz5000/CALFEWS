...
class Delta():
  __slots__ = ["T", "T_short", "model_mode", "key", "forecastSCWYT", "forecastSJWYT", "last_year_vamp", "min_outflow", "export_ratio",
               "pump_max", "omr_reqr", "omr_addition", "rio_vista_min", "san_joaquin_min", "san_joaquin_min_flow",
               "vamp_flows", "new_vamp_rule_old", "new_vamp_rule", "san_joaquin_add", "san_joaquin_export_ratio",
               ...
               "first_empty_day_SWP", "first_empty_day_CVP", "final_allocation_swp", "final_allocation_cvp", 
               'omr_regression', 'x2_dict', 'expected_depletion', 'max_tax_free', 'rio_gains', 'forecastSWPPUMP', 
               'vernalis_gains', 'delta_uncontrolled_remaining', 'forecastCVPPUMP', 'total_inflow', 'iter_count',
               'remaining_outflow', 'expected_outflow']
  ...
  def __init__(self, model, name, key, model_mode):
    ...
    self.gains_sac = model.df.SAC_gains * cfs_tafd
    self.gains_sj = model.df.SJ_gains * cfs_tafd
    self.depletions = model.df.delta_depletions * cfs_tafd
    self.vernalis_flow = np.zeros(self.T)
    self.eastside_streams = model.df.EAST_gains * cfs_tafd
    self.inflow = np.zeros(self.T)
    self.ccc = model.df.CCC_pump * cfs_tafd
    self.barkerslough = model.df.BRK_pump *cfs_tafd
    ...
    self.annual_HRO_pump = np.zeros(model.number_years)
    self.annual_TRP_pump = np.zeros(model.number_years)
  ...
  def meet_OMR_requirement(self, t, m, dowy, year, cvp_m, swp_m):
    wyt = self.forecastSCWYT
    ...
    if year < self.omr_rule_start:
      omrRequirement = fish_trigger_adj
    elif year == self.omr_rule_start and m < 12:
      omrRequirement = fish_trigger_adj
    else:
  ...
  def hypothetical_pumping(self, capacity_max, supply_max, pump_trigger, fraction, t, m):
    wyt = self.forecastSCWYT
    if pump_trigger == 0:
      if supply_max > 0.0:
  ...
  def calc_flow_bounds(self, t, d, m, dowy, year, dowy_eom, cvp_max, swp_max, cvp_max_alt, swp_max_alt, cvp_release, swp_release, cvp_AS, swp_AS, cvp_flood, swp_flood, swp_over_dead_pool, cvp_over_dead_pool, swp_flood_volume, cvp_flood_volume, swp_fillup, cvp_fillup):
    ...
    tax_free_flood = max(self.max_tax_free[wyt]['cvp'][dowy] + self.max_tax_free[wyt]['swp'][dowy] - self.max_tax_free[wyt]['cvp'][dowy_eom[year][3]] - self.max_tax_free[wyt]['swp'][dowy_eom[year][3]], 1.0)#from this date until the end of the 'flood control' season (April 1)
    tax_free_oct_nov = max(self.max_tax_free[wyt]['cvp'][dowy] + self.max_tax_free[wyt]['swp'][dowy] - self.max_tax_free[wyt]['cvp'][dowy_eom[year][10]] - self.max_tax_free[wyt]['swp'][dowy_eom[year][10]], 1.0) #from this date until the end of november (i.e. baseflow period before reservoir refil usually begins)
    ...
    cvp_tax_free_pumping, swp_tax_free_pumping = self.meet_OMR_requirement(t, m, dowy, year, cvp_tax_free_pumping, swp_tax_free_pumping)
  ...
  # def find_release_flood_prep(self, m, dowy, days_in_month, proj_surplus, numdays, min_release, key):
  ...
  def find_release(self, t, m, dowy, year, days_in_month, dowy_eom, cvp_max, swp_max, cvpAS, swpAS, cvp_release, swp_release, proj_surplus, max_pumping):
  ...
  ##when the inflow/export 'tax' is lowest)
    ...
    for project in ['cvp', 'swp']:
      ...
      running_days = dowy_eom[year][monthcounter] - dowy
      ...
      if monthcounter == 3 or monthcounter == 4:
        ...
        pumping['cvp'] = self.pump_max['cvp']['intake_limit'][5] * cfs_tafd
      if year > self.omr_rule_start:
        pumping['swp'] = min(pumping['swp'], max_pumping['swp'][monthcounter]/days_in_month[year][monthcounter])
        pumping['cvp'] = min(pumping['cvp'], max_pumping['cvp'][monthcounter]/days_in_month[year][monthcounter])
      if year == self.omr_rule_start and m == 12:
        pumping['swp'] = min(pumping['swp'], max_pumping['swp'][monthcounter]/days_in_month[year][monthcounter])
        pumping['cvp'] = min(pumping['cvp'], max_pumping['cvp'][monthcounter]/days_in_month[year][monthcounter])
      ...
      total_uncontrolled = proj_surplus[project][monthcounter]*running_days/days_in_month[year][monthcounter]
      self.delta_uncontrolled_remaining += total_uncontrolled
      #water available to pump w/o exceeding the I/E tax
      untaxed_releases = max(self.max_tax_free[wyt][project][dowy] - self.max_tax_free[wyt][project][dowy_eom[year][monthcounter]+1] - total_uncontrolled, 0.0)
      ...
      for x in range(0,12):
        ...
        running_days += days_in_month[year+cross_counter_y][monthcounter]
        if year > self.omr_rule_start:
          pumping['swp'] = min(pumping['swp'], max_pumping['swp'][monthcounter]/days_in_month[year+cross_counter_y][monthcounter])
          pumping['cvp'] = min(pumping['cvp'], max_pumping['cvp'][monthcounter]/days_in_month[year+cross_counter_y][monthcounter])
        if year == self.omr_rule_start and m == 12:
          pumping['swp'] = min(pumping['swp'], max_pumping['swp'][monthcounter]/days_in_month[year+cross_counter_y][monthcounter])
          pumping['cvp'] = min(pumping['cvp'], max_pumping['cvp'][monthcounter]/days_in_month[year+cross_counter_y][monthcounter])
        ...
        self.delta_uncontrolled_remaining += total_uncontrolled
        #water available to pump w/o exceeding the I/E tax
        dowy_begin = dowy_eom[year+cross_counter_y][monthcounter] - days_in_month[year+cross_counter_y][monthcounter]
        dowy_end = dowy_eom[year+cross_counter_y][monthcounter]
        ...
        #total pumping from uncontrolled or untaxed flows
        if monthcounter == 7 or monthcounter == 8:
          untaxed += min(total_uncontrolled + min(running_storage, untaxed_releases), pumping[project]*days_in_month[year+cross_counter_y][monthcounter])
        else:
          untaxed += min(total_uncontrolled + min(running_storage, untaxed_releases), pumping[project]*days_in_month[year+cross_counter_y][monthcounter])
	    ##space for pumping that exceeds the I/E tax
		#########
        #if monthcounter > 5 or monthcounter == 0:
          #taxable_space += max(pumping[project]*days_in_month[year+cross_counter_y][monthcounter] - untaxed, 0.0)
        #elif monthcounter > 0 and monthcounter < 3:
          #taxable_space2 += max(pumping[project]*days_in_month[year+cross_counter_y][monthcounter] - untaxed, 0.0)
        #elif monthcounter == 5:
          #taxable_space2 += max(pumping[project]*days_in_month[year+cross_counter_y][monthcounter] - untaxed, 0.0)
        #update running storage
        running_storage -= min(running_storage, untaxed_releases)
  ...
  def step(self, t, d, m, dowy, da, wateryear, year, cvp_flows, swp_flows, swp_pump, cvp_pump, swp_AS, cvp_AS):
    ...
    cvp_max, swp_max = self.meet_OMR_requirement(t, m, dowy, year, cvp_max, swp_max)
    self.TRP_pump[t], self.HRO_pump[t] = self.meet_OMR_requirement(t, m, dowy, year, self.TRP_pump[t], self.HRO_pump[t])
  ...
  def create_flow_shapes_omr(self, model):
    ...
    fnf_series = np.zeros(model.T_short)
