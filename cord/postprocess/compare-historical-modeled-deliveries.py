...
canal_results = pd.read_csv('cord/data/results/canal_results_validation.csv', index_col=0, parse_dates=True)
...
fig.savefig('cord/figs/compareCVP/friant_allContractors.png', dpi=150)
...
ax.set_ylabel('Modeled deliveries (taf/yr)')
ax.set_xlabel('Historical deliveries (taf/yr)')
fig.savefig('cord/figs/compareCVP/friant_all.png', dpi=150)
...
for k in range(3):
  ...
  for i in range(2):
    for j in range(3):
      ...
      ax.set_ylabel('Modeled deliveries (taf/yr)')
      ax.set_xlabel('Historical deliveries (taf/yr)')
  plt.savefig('cord/figs/compareCVP/annual_scatter_%s.png' % name, dpi=150)
...
for k in range(3):
  ...
  for i in range(2):
    for j in range(3):
      ...
      ax.set_xlabel('Date')
  plt.savefig('cord/figs/compareCVP/annual_timeseries_%s.png' % name, dpi=150)
...
for k in range(3):
  ...
  for i in range(2):
    for j in range(3):
      ...
      ax.set_ylabel('Modeled deliveries (taf/yr)')
      ax.set_xlabel('Historical deliveries (taf/yr)')
  plt.savefig('cord/figs/compareCVP/monthly_scatter_%s.png' % name, dpi=150)
