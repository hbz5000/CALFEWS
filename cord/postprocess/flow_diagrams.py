...
canal3, canal_aggregate_Wateryear3, canal_aggregate_Wateryear_Month3, ind_canal_waterway3, ind_canal_recipient3, ind_canal_flowtype3 = \
            load_model_data.load_canal_data('cord/data/results/FKC_capacity_wy2017__LWT_inleiubank_ARV_0/canal_results_simulation.csv')
canal4, canal_aggregate_Wateryear4, canal_aggregate_Wateryear_Month4, ind_canal_waterway4, ind_canal_recipient4, ind_canal_flowtype4 = \
            load_model_data.load_canal_data('cord/data/results/FKC_capacity_wy2017__LWT_inleiubank_ARV_025/canal_results_simulation.csv')
...
### get district data
for folder in ['FKC_capacity_wy2017','FKC_capacity_rehab_full','FKC_capacity_wy2017__LWT_inleiubank_ARV_0','FKC_capacity_rehab_full__LWT_inleiubank_ARV_0','FKC_capacity_wy2017__LWT_inleiubank_ARV_withRecharge_100','FKC_capacity_rehab_full__LWT_inleiubank_withRecharge_100']:
...
# # screen_districts = False
# # screen for particular districts
# dum = df_reorg.loc[(df_reorg.District=='PIX')&((df_reorg.Label=='Friant')|(df_reorg.Label=='OtherSW')),:].groupby('Wateryear').sum()
# plt.hist(dum.Volume, color='k', kernel='epa')
# # sns.kdeplot(turnout_aggregate_Wateryear2.loc[:,ind_district].sum(axis=1), color=cols[1], kernel='epa', label='Friant rehab')
# # sns.kdeplot(turnout_aggregate_Wateryear3.loc[:,ind_district].sum(axis=1), color=cols[2], kernel='epa', label='LWT bank')
# # sns.kdeplot(turnout_aggregate_Wateryear4.loc[:,ind_district].sum(axis=1), color=cols[3], kernel='epa', label='Friant rehab + LWT bank')
# ax.set_xlim([0,800])
