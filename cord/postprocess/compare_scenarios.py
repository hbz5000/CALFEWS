...
plt.savefig('cord/figs/compareScenarios/%s/compare_%s_all.png' % (folder_name,flow_type), dpi=150)
    ...
    for j in range(3):
      ...
      if (plot_type == 'timeseries'):
        ...
        if (aggregation == 'wy'):
          ...
          ax.set_xlabel('Scenario 1 ' + flow_type + ' (taf/month)')
          ax.set_ylabel('Scenario 2 ' + flow_type + ' (taf/month)')
  plt.savefig('cord/figs/compareScenarios/%s/compareFriant_%s_%s_%s_%s.png' % (folder_name, flow_type, plot_type, aggregation, name), dpi=150)
  ...
  for i in range(2):
    for j in range(3):
      ...
      ax.annotate(d[i][j], xy=(ax.get_xlim()[0] + (ax.get_xlim()[1] - ax.get_xlim()[0]) * 0.91,
                               ax.get_ylim()[0] + (ax.get_ylim()[1] - ax.get_ylim()[0]) * 0.95))
  plt.savefig('cord/figs/compareScenarios/%s/compareKNR_%s_%s_%s_%s.png' % (folder_name, flow_type, plot_type, aggregation, name), dpi=150)
...
for k in range(len(d_list)):
  ...
  for i in range(2):
    for j in range(3):
      ...
      ax.set_title(d[i][j])
plt.savefig('cord/figs/compareScenarios/%s/pdf_%s.png' % (folder_name,flow_type), dpi=150)
