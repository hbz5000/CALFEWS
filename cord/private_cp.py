...
class Private():
  __slots__ = ["key", "name", "turnback_use", "district_list", "crop_list", "in_district_direct_recharge",
               "banking_risk_level", "recovery_fraction", "pump_out_fraction", "seepage", "crop_maturity", "MDD", "zone",
               "urban_profile", "contract_fractions", "irrdemand", "deliveries", "contract_list_all",
               "non_contract_delivery_list", "current_balance", "paper_balance", "turnback_pool", "projected_supply",
               "carryover", "recharge_carryover", "delivery_carryover", "contract_carryover_list", "daily_supplies",
               "supply_list", "annual_supplies", "daily_supplies_full", "demand_days", "annualdemand", "dailydemand",
               "dailydemand_start", "per_acre", "total_acreage", "unmet_et", "last_days_demand_regression_error",
               "annual_pumping", "use_recharge", "use_recovery", "extra_leiu_recovery", "max_recovery",
               "max_leiu_exchange", "total_banked_storage", "direct_recovery_delivery", "recharge_rate", "thismonthuse",
               "monthusecounter", "monthemptycounter", "current_recharge_storage", "acreage", "contract_fraction",
               "private_fraction", "contract_list", "initial_planting", "has_pesticide", "has_pmp", 'reservoir_contract', 
               'monthlydemand', 'turnout_list', 'seasonal_connection', 'carryover_rights', 'delivery_risk_rate', 'must_fill', 
               'delivery_risk', 'target_annual_demand', 'agedemand', 'age_death', 'max_direct_recharge', 'max_leiu_recharge', 
               'pumping', 'demand_auto_errors', 'ytd_pumping', 'regression_errors_timeseries', 'hist_demand_dict', 
               'ytd_pumping_int', 'regression_errors', 'delivery_percent_coefficient', 'regression_percent', 
               'k_close_wateryear', 'turnback_purchases', 'turnback_sales', 'iter_count', 'district_list_length',
               'dynamic_recharge_cap', 'annual_private_pumping', 'recovery_capacity_remain', 'delivery_location_list']
  ...
  def __init__(self, model, name, key, land_fraction):
    ...
    self.district_list_length = len(self.district_list)
...
#####################################################################################################################
  def permanent_crop_growth(self, wateryear, days_in_month, non_leap_year):
    ...
    for x in self.district_list:
      ...
      for v in self.crop_list:
        ...
        self.per_acre[x][v]['mature'] = 0.0
        self.per_acre[x][v]['immature'] = 0.0
    self.find_baseline_demands(days_in_month, non_leap_year)
		
  def find_baseline_demands(self, days_in_month, non_leap_year):
    self.monthlydemand = {}
    self.agedemand = {}
  ...
  def calc_demand(self, wateryear, year, da, m, m1, wyt, days_in_month, non_leap_year):
    ...
    monthday = days_in_month[year][m-1]
    for x in self.district_list:
	...
	#pro-rate this month's demand based on the day of the month when calculating remaining annual demand
      ...
      if m > 9:
        for monthloop in range(m, 12):
          self.annualdemand[x] += max(self.monthlydemand[x][wyt][monthloop]*days_in_month[year][monthloop],0.0)

        for monthloop in range(0,9):
          self.annualdemand[x] += max(self.monthlydemand[x][wyt][monthloop]*days_in_month[year+1][monthloop], 0.0)

      else:
        for monthloop in range(m, 9):
          self.annualdemand[x] += max(self.monthlydemand[x][wyt][monthloop]*days_in_month[year][monthloop], 0.0)

  def get_urban_demand(self, t, m, da, wateryear, year, sri, dowy, total_delta_pumping, allocation_change, model_mode, dowy_eom):
      ...
      if m == 10 and da == 1:
        ...
        for monthloop in range(0,12):
          ...
          if monthcounter > 11:
            monthcounter -= 12
            cross_counter_y = 1
          start_next_month = dowy_eom[year+cross_counter_y][monthcounter] + 1
          for wyt in ['W', 'AN', 'BN', 'D', 'C']:
            for district in self.district_list:
  ...
  def find_pre_flood_demand(self, wyt, year, days_in_month):
    #calculates an estimate for water use in the Oct-Dec period (for use in recharge_carryover calculations), happens Oct 1
    self.pre_flood_demand = self.monthlydemand[wyt][9]*days_in_month[year][9] + self.monthlydemand[wyt][10]*days_in_month[year][10] + self.monthlydemand[wyt][11]*days_in_month[year][11]
...
#####################################################################################################################
  ...
  def apply_paper_balance(self, key, wyt, wateryear):
    ...
    if remaining_paper > 0.0:
      ...
      if remaining_paper > 0.0:
        for x in self.district_list:
          ind_paper_balance[x] += remaining_paper/self.district_list_length
    for x in self.district_list:
  ...
  def apply_paper_balance_urban(self, key, wyt, wateryear):
    ...
    if self.paper_balance[key] > 0.0:
      ...
      if self.paper_balance[key] > total_used:
        for xx in self.district_list:
          self.projected_supply[xx][key] += (self.paper_balance[key] - total_used)/self.district_list_length
    else:
      for xx in self.district_list:
        if self.projected_supply[xx][key] > 0.0:
...
#####################################################################################################################
  ...
  def open_recovery(self, t, dowy, wyt, wateryear, use_delivery_tolerance, additional_carryover, number_years):
    ...
    if use_delivery_tolerance:
      ...
      delivery_tolerance = 0.0
      self.target_annual_demand = 999999.9*np.ones(number_years)
      #for district in self.district_list:
  ...
  def open_recharge(self,t,m,da,wateryear,year,numdays_fillup, numdays_fillup2, key, wyt, reachable_turnouts, additional_carryover, contract_allocation, days_in_month):
    ...
    ###Find projected recharge available to district
      ...
      #total_recharge_capacity = (self.max_direct_recharge[0] + self.max_leiu_recharge[m])*(days_in_month[year][m]-da)
        ...
        #for future_month in range(m+1,9):
          #total_recharge_capacity += self.max_direct_recharge[future_month - m]*days_in_month[year][future_month] + self.max_leiu_recharge[future_month]*days_in_month[year+1][future_month]
      #elif m > 8:
        #for future_month in range(m+1,12):
          #total_recharge_capacity += self.max_direct_recharge[future_month - m]*days_in_month[year][future_month] + self.max_leiu_recharge[future_month]*days_in_month[year+1][future_month]
        #for future_month in range(0,9):
          #total_recharge_capacity += self.max_direct_recharge[future_month - m]*days_in_month[year+1][future_month] + self.max_leiu_recharge[future_month]*days_in_month[year+1][future_month]
      #else:
    ...
    if numdays_fillup < 365.0:
      ...
      this_month_recharge = (self.max_direct_recharge[0] + self.max_leiu_recharge[0])*min(days_in_month[year][m] - da,days_left)
      this_month_recharge2 = (self.max_direct_recharge[0] + self.max_leiu_recharge[0])*min(days_in_month[year][m] - da,days_left2)
      total_recharge += this_month_recharge
      total_recharge2 += this_month_recharge2
	  #days before fillup remaining after current month
      days_left -= (days_in_month[year][m] - da)
      days_left2 -= (days_in_month[year][m] - da)
	...
	###loop through future months to determine how much water can be recharged before reservoir fills
      ...
      while (monthcounter + monthcounter_loop) < 11 and days_left > 0.0:
        ...
        this_month_recharge = (self.max_direct_recharge[monthcounter+monthcounter_loop] + self.max_leiu_recharge[monthcounter+monthcounter_loop])*min(days_in_month[year+next_year_counter][m+monthcounter],days_left)
        total_recharge += this_month_recharge
     
        days_left -= days_in_month[year+next_year_counter][m+monthcounter]
      ...
      while (monthcounter + monthcounter_loop) < 11 and days_left2 > 0.0:
        ...
        if (monthcounter + m) > 11:
 	    ...
 	    # continue to tabulate how much water can be recharged between now & reservoir fillup (future months)
        this_month_recharge2 = (self.max_direct_recharge[monthcounter+monthcounter_loop] + self.max_leiu_recharge[monthcounter+monthcounter_loop])*min(days_in_month[year+next_year_counter][m+monthcounter],days_left2)
        total_recharge2 += this_month_recharge2
     
        days_left2 -= days_in_month[year+next_year_counter][m+monthcounter]
  ...
  def set_turnback_pool(self, key, year, additional_carryover, days_in_month):
    ...
    for month_count in range(0, 6):
      ...
      total_recharge_ability += self.max_direct_recharge[month_count]*days_in_month[year][month_count +3] + self.max_leiu_recharge[month_count]*days_in_month[year][month_count +3]
      ...
      if search_type == "flood":
        for x in self.district_list:
          if contract_deliveries > 0.0:
            self.deliveries[x][y.name + '_flood'][wateryear] += recharge_deliveries/self.district_list_length
            self.deliveries[x][delivery_location + '_recharged'][wateryear] += recharge_deliveries/self.district_list_length
            self.deliveries[x][y.name + '_flood_irrigation'][wateryear] += direct_deliveries/self.district_list_length
